...
def clone_and_read_sources(clone_url: str, ref: str) -> tuple[str, dict[str, str]]:
    ...
    try:
        sandbox.commands.run(
            f"git clone --depth 1 --branch {ref} {clone_url} /home/user/repo",
            timeout=COMMAND_TIMEOUT,
        )
        listed = sandbox.commands.run(
            "cd /home/user/repo && git ls-files '*.py' '*.js' '*.ts' '*.tsx' '*.jsx'",
            timeout=COMMAND_TIMEOUT,
        )
        paths = [p.strip() for p in listed.stdout.splitlines() if p.strip()][:MAX_FILES]
        files: dict[str, str] = {}
        for rel in paths:
